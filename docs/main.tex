...
\section{System model proposals}

\subsection{Learned compression with a VQ-VAE}
...
\subsubsection{VQ-VAE training losses (codebook and commitment)}
...
\subsection{Deterministic compression with FWHT sparsification}
As an alternative to learned quantization, we consider a deterministic transform coder based on the
\emph{Fast Walsh--Hadamard Transform (FWHT)}. The method applies the same front-end preprocessing used above
(decimation, standardization, and the optional non-linear mapping) to obtain $\bm{\sigma}_t\in\mathbb{R}^{N'}$ and
transmits the corresponding side-information bits $b_t$. The main bitstream $b_s$ is then formed by sparsifying the
FWHT coefficients of $\bm{\sigma}_t$.

\paragraph{Hadamard transform.}
Let $N_H$ be a power of two. If $N'$ is not a power of two, we zero-pad $\bm{\sigma}_t$ to length
$N_H = 2^{\lceil \log_2 N' \rceil}$ (and discard padded bins after inversion). Let $H_{N_H}\in\{\pm1\}^{N_H\times N_H}$
denote the Hadamard matrix and use the orthonormal transform $\frac{1}{\sqrt{N_H}}H_{N_H}$. The sequency-domain
coefficients are
\begin{equation}
\mathbf{c}_t = \frac{1}{\sqrt{N_H}}\,H_{N_H}\,\bm{\sigma}_t.
\end{equation}

\paragraph{Top-$K$ sparsification and bitstream $b_s$.}
Define $\Omega_t$ as the index set of the $K$ largest-magnitude coefficients of $\mathbf{c}_t$.
We form a sparse vector $\tilde{\mathbf{c}}_t$ such that $(\tilde{\mathbf{c}}_t)_i = (\mathbf{c}_t)_i$ for $i\in\Omega_t$
and zero otherwise. The transmitted payload $b_s$ contains (i) the locations $\Omega_t$ (e.g., as an index list or a bitmask)
and (ii) quantized coefficient values $\mathcal{Q}((\mathbf{c}_t)_i)$ for $i\in\Omega_t$.
In practice, $\Omega_t$ can be computed efficiently without a full sort (e.g., via selection algorithms such as Quickselect).

\paragraph{Reconstruction.}
The cloud reconstructs the transformed PSD by the inverse FWHT:
\begin{equation}
\hat{\bm{\sigma}}_t = \frac{1}{\sqrt{N_H}}\,H_{N_H}\,\tilde{\mathbf{c}}_t,
\end{equation}
and then applies the same inverse processing as before using $b_t$:
$\hat{\mathbf{s}}_t=g_{\mathrm{nl}}^{-1}(\hat{\bm{\sigma}}_t)$,
$\hat{\mathbf{S}}'_t=\mathrm{DeStd}(\hat{\mathbf{s}}_t;\mathbf{t}_t)$,
and $\hat{\mathbf{S}}_t=\mathcal{U}_{M/K}(\hat{\mathbf{S}}'_t)$.

\paragraph{Complexity and operating point.}
FWHT has $\mathcal{O}(N_H\log N_H)$ time complexity and uses only additions/subtractions.
The rate--distortion trade-off is controlled mainly by $K$ and the coefficient quantizer $\mathcal{Q}(\cdot)$,
with no training required.

\subsection{KL/PCA compression over frequency grid}
As a third model, we include a \emph{Karhunen--Lo\`eve (KL) / PCA} frequency-domain representation
implemented as a module-based codec in the unified library. The method uses a covariance projection over
the frequency grid, projects PSD snapshots to a low-rank subspace, and reconstructs from retained
coefficients in that subspace.

\paragraph{KL basis on weighted frequency grid.}
Given frequencies $\{f_i\}_{i=1}^{M}$ and quadrature weights $\{w_i\}_{i=1}^{M}$, define the weighted
kernel matrix
\begin{equation}
K_w(i,j) = k(f_i,f_j)\sqrt{w_iw_j},
\end{equation}
with $k(\cdot,\cdot)$ chosen as Mat\'ern-$3/2$ (single or composite scale). Let
$K_w \mathbf{v}_n=\lambda_n \mathbf{v}_n$, sorted by descending $\lambda_n$. Retaining $R$ dominant modes,
the KL feature matrix is
\begin{equation}
\Phi_{\mathrm{KL}}(i,n)=\sqrt{\lambda_n}\,\phi_n(f_i), \quad
\phi_n(f_i)=\frac{v_n(i)}{\sqrt{w_i}}.
\end{equation}

\paragraph{Low-rank PSD model and posterior reconstruction.}
For each frame, the excess PSD is modeled as
\begin{equation}
\mathbf{s}_t \approx \Phi_{\mathrm{KL}} \boldsymbol{\xi}_t,\qquad
\boldsymbol{\xi}_t \sim \mathcal{N}(\mathbf{0},I_R),
\end{equation}
with observation model
\begin{equation}
\mathbf{y}_t = A\boldsymbol{\xi}_t + \boldsymbol{\varepsilon}_t,\qquad
A\equiv \Phi_{\mathrm{KL}},\quad
\boldsymbol{\varepsilon}_t\sim\mathcal{N}(\mathbf{0},\sigma_\varepsilon^2 I).
\end{equation}
The posterior in coefficient space is Gaussian:
\begin{align}
\Sigma_{\xi|y,t} &= \left(I_R + \frac{A^\top A}{\sigma_\varepsilon^2}\right)^{-1},\\
\mu_{\xi|y,t} &= \Sigma_{\xi|y,t}\frac{A^\top y_t}{\sigma_\varepsilon^2},
\end{align}
and reconstructed PSD mean is
\begin{equation}
\hat{\mathbf{s}}_t = A\,\mu_{\xi|y,t}.
\end{equation}
This provides a deterministic dimensionality reduction path (KL/PCA basis) plus uncertainty-aware
inference in the reduced space.

\subsubsection*{Implementation mapping (official three-model scope)}
\begin{center}
\begin{tabular}{|l|p{0.66\textwidth}|}
\hline
\textbf{Model} & \textbf{Primary implementation path in repository} \\ \hline
KL/PCA over frequency grid &
\texttt{psd\_compression/kl\_pca/model.py} and \texttt{psd\_compression/kl\_pca/tasks.py} \\ \hline
Learned VAE compression &
\texttt{VAE\_implementation/scripts/training/01\_preprocess.py} through
\texttt{VAE\_implementation/scripts/analysis/06\_codec\_benchmark.py} (plus UDP scripts) \\ \hline
Deterministic FWHT sparsification &
\texttt{psd\_compression/fwht/codec.py} and \texttt{psd\_compression/fwht/tasks.py}
(reference notebook kept at \texttt{notebooks/examples/fwht.ipynb}) \\ \hline
\end{tabular}
\end{center}
